...
function duplicate(videoId) {
  ...
  return this.info(videoId)
    .then(function(props) {
      ...
      return getDownloadLinks(props.body.download);
    })

    .then(function(link) {
      if (!link) throw new Error('no download link [original, hd, sd]');
      isHD = link.quality == 'hd';
      return getRedirectLink(link.link);
    })

    .then(function(redirectLink) {
      return hyperquest(redirectLink);
    })
