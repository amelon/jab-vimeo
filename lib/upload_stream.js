 ...
 *   - {string} res.videoId
 *   - {Object} body - json response of name update of the video
 */
function uploadStream(stream) {
  var filename = determineFilename(stream),
      _this    = this, // this is vimeo client instance
      completeUri, videoId;

          // get upload ticket
  return prepareUpload(_this)
    ...
    .then(function(props) {
      ...
      return patch(_this, videoId, { name: filename });
    })
 ...
 * @return {<Promise>Object} object containing body, res
 */
function patch(vimeo, videoId, data) {
  return new Promise(function(resolve, reject) {
    var options = {
      method: 'PATCH'
    , path: '/videos/' + videoId
